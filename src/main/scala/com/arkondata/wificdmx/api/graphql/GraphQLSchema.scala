...
object GraphQLSchema {
  ...
  val WifiPageType: ObjectType[Unit, PagedResult[WifiPoint]] =
    ObjectType("WifiPointsPage", "Paginated list of WiFi points",
      ...
      Field("wifiPoints", WifiPageType,
        ...
        resolve     = ctx =>
          ctx.arg(AlcaldiaArg) match {
            case Some(a) => service.getByAlcaldia(a, ctx.arg(PageArg), ctx.arg(PageSizeArg))
                              .map(_.fold(e => throw new Exception(e.toString), identity))
            case None    => service.getAll(ctx.arg(PageArg), ctx.arg(PageSizeArg))
                              .map(_.fold(e => throw new Exception(e.toString), identity))
          }
      ),

      Field("wifiPoint", OptionType(WifiPointType),
        arguments   = List(IdArg),
        description = Some("Find a single WiFi point by its ID"),
        resolve     = ctx =>
          service.getById(ctx.arg(IdArg))
            .map(_.fold(_ => None, Some(_)))
      ),

      Field("nearbyPoints", WifiPageType,
        ...
        resolve     = ctx =>
          service.getNearby(ctx.arg(LatArg), ctx.arg(LonArg),
                            ctx.arg(PageArg), ctx.arg(PageSizeArg))
            .map(_.fold(e => throw new Exception(e.toString), identity))
      )
    ))
